...
//! intermediates the verifier may build through, and a policy that decides
...
fn main() {
    ...
    // available for the verifier to build a path through, and each one still
    ...
    match verifier.validate_with_diagnostics(&chain.leaf, &intermediates, &mut |_| {}) {
    ...
    let result = verifier.validate_with_diagnostics(&chain.leaf, &CertificateStore::new(), &mut |_| {});
    println!(
