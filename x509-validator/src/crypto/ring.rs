...
// name for paths written inside it; refer to the crate as `::ring`.
...
/// public-key curve OID) to the matching ring verification algorithm.
...
/// ring ships no ECDSA-with-SHA512 verification algorithm for either curve,
/// so those pairings return `None` and surface as `CryptoError::InvalidKey`.
...
#[derive(Debug)]
struct RingPublicKey {
    algorithm: &'static dyn VerificationAlgorithm,
    spki_der: Vec<u8>,
}

impl PublicKey for RingPublicKey {
